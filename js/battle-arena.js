...
async function inicializarArenaBattle() {
    ...
    try {
        if (esModoBossArena()) {
            ...
            const estadoRestaurado = restaurarEstadoBossArena();
            if (estadoRestaurado) {
                await cargarMovimientosEquipoJugadorArena();
                guardarEquipoArenaEnStorages();
            } else if (existeSesionBossLocalArena()) {
                sincronizarSesionBossArena({
                    boss_session_token: sessionStorage.getItem(BATTLE_BOSS_SESSION_TOKEN_KEY) || null,
                    expira_en: sessionStorage.getItem(BATTLE_BOSS_SESSION_EXPIRES_KEY) || null,
                    boss: arenaBossActual,
                    evento: arenaBossEventoActual
                });
                generarBossEnemigoArena(arenaBossActual);
                guardarEstadoBossArena();
            } else {
                const bossData = await iniciarSesionBossArena();
                if (Array.isArray(bossData?.snapshot_equipo) && bossData.snapshot_equipo.length === 6) {
                    arenaPlayerTeam = bossData.snapshot_equipo
                        .slice(0, 6)
            ...
            await iniciarSesionGymArenaDesdeStorage();
        } else {
            await cargarEquipoJugadorArena();
...
function configurarEventosArena() {
    ...
    if (btnSalir) btnSalir.addEventListener("click", manejarSalidaArena);
    ...
    if (btnVolverBattle) {
        btnVolverBattle.addEventListener("click", manejarSalidaArena);
    }

    if (btnReintentar) {
        btnReintentar.addEventListener("click", manejarReintentoArena);
    }
}
...
/* =========================================================
   MODO / BOSS
========================================================= */
function obtenerModoArenaActual() {
    try {
        ...
        console.warn("No se pudo leer el modo de arena desde la URL:", error);
    }

    const guardado = String(sessionStorage.getItem(BATTLE_ARENA_MODE_KEY) || "arena").trim().toLowerCase();
    return ["arena", "boss", "gym"].includes(guardado) ? guardado : "arena";
}
...
function existeSesionBossLocalArena() {
    ...
    return Boolean(token && boss);
}

function construirExpiraBossArena(ttlSegundos = 0, expiraEn = null) {
    if (expiraEn) return String(expiraEn);
    const ttl = Number(ttlSegundos || 0);
    if (ttl <= 0) return null;
    return new Date(Date.now() + (ttl * 1000)).toISOString();
}

function construirEstadoSerializableBossArena() {
    return {
        boss_session_token: arenaBattleSessionToken || null,
        expira_en: arenaBattleSessionExpiraEn || null,
        ttl_segundos: Number(arenaBattleSessionTtlSegundos || 0),
        turno: Number(arenaTurno || 1),
        player_index: Number(arenaPlayerIndex || 0),
        enemy_index: Number(arenaEnemyIndex || 0),
        boss: arenaBossActual || null,
        evento: arenaBossEventoActual || null,
        player_team: Array.isArray(arenaPlayerTeam) ? arenaPlayerTeam : [],
        enemy_team: Array.isArray(arenaEnemyTeam) ? arenaEnemyTeam : []
    };
}

function guardarEstadoBossArena() {
    if (!esModoBossArena()) return;

    if (arenaCombatEnded || !arenaBattleSessionToken) {
    ...
    try {
        sessionStorage.setItem(
            BATTLE_BOSS_COMBAT_STATE_KEY,
            JSON.stringify(construirEstadoSerializableBossArena())
        );
    } catch (error) {
        console.warn("No se pudo guardar el estado local del Boss:", error);
    }
}

function restaurarEstadoBossArena() {
    const estado = leerJSONArena(BATTLE_BOSS_COMBAT_STATE_KEY, null);
    if (!estado || typeof estado !== "object") return false;

    const token = String(estado.boss_session_token || sessionStorage.getItem(BATTLE_BOSS_SESSION_TOKEN_KEY) || "").trim();
    if (!token) return false;

    if (!Array.isArray(estado.player_team) || !Array.isArray(estado.enemy_team)) {
        return false;
    }

    if (estado.player_team.length !== 6 || estado.enemy_team.length < 1) {
        return false;
    }

    arenaBattleSessionToken = token;
    arenaBattleSessionTtlSegundos = Number(estado.ttl_segundos || 0);
    arenaBattleSessionExpiraEn = estado.expira_en || sessionStorage.getItem(BATTLE_BOSS_SESSION_EXPIRES_KEY) || null;
    arenaTurno = Math.max(1, Number(estado.turno || 1));
    arenaBossActual = estado.boss || arenaBossActual;
    arenaBossEventoActual = estado.evento || arenaBossEventoActual;
    arenaPlayerTeam = estado.player_team.map((pokemon, index) => normalizarPokemonArena(pokemon, "player", index));
    arenaEnemyTeam = estado.enemy_team.map((pokemon, index) => normalizarPokemonArena(pokemon, "enemy", index));
    arenaPlayerIndex = Math.max(0, Math.min(arenaPlayerTeam.length - 1, Number(estado.player_index || 0)));
    arenaEnemyIndex = Math.max(0, Math.min(arenaEnemyTeam.length - 1, Number(estado.enemy_index || 0)));

    sessionStorage.setItem(BATTLE_BOSS_SESSION_TOKEN_KEY, arenaBattleSessionToken);
    if (arenaBattleSessionExpiraEn) {
        sessionStorage.setItem(BATTLE_BOSS_SESSION_EXPIRES_KEY, arenaBattleSessionExpiraEn);
    }

    if (arenaBossActual) {
        sessionStorage.setItem(BATTLE_BOSS_DATA_KEY, JSON.stringify(arenaBossActual));
    }
    if (arenaBossEventoActual) {
        sessionStorage.setItem(BATTLE_BOSS_EVENT_KEY, JSON.stringify(arenaBossEventoActual));
    }
...
async function manejarSalidaArena() {
    ...
    if (esModoGymArena()) {
        if (!arenaCombatEnded) {
            try {
                await cancelarGymArenaApi();
            } catch (error) {
                console.warn("No se pudo cancelar la sesión del Gym al salir:", error);
            }
        }
...
function manejarReintentoArena() {
    detenerAutoTurnoArena();
...
function actualizarHeroModoArena() {
    const badge = document.querySelector(".arena-hero-badge");
    const title = document.querySelector(".arena-title");
    const subtitle = document.querySelector(".arena-subtitle");
    const enemyKicker = document.querySelector(".arena-side-card.enemy .arena-side-kicker");
    const enemySection = document.querySelector(".arena-team-preview-wrap > div:last-child .arena-subsection-title");
    const btnBack = document.getElementById("btnArenaVolverBattle");
    const btnRetry = document.getElementById("btnArenaReintentar");

    actualizarMenuModoArena();

    if (esModoBossArena()) {
        if (badge) badge.textContent = tSafeArena("battle_arena_badge_boss");
        if (title) title.textContent = tSafeArena("battle_arena_title_boss");
    ...
    if (esModoGymArena()) {
        const gymLeader = arenaGymActual?.leader || arenaGymActual?.lider_nombre || "Gym Leader";
        const gymBadge = arenaGymActual?.badge || arenaGymActual?.medalla_nombre || "Gym Badge";
        const regionName = arenaGymActual?.regionName || arenaGymActual?.region_nombre || arenaGymActual?.region || "Region";
        if (badge) badge.textContent = `Gym Challenge · ${regionName}`;
        if (title) title.textContent = `${gymLeader} Arena`;
        if (subtitle) subtitle.textContent = `Face ${gymLeader} and claim the ${gymBadge} using your main team of 6 Pokémon.`;
        if (enemyKicker) enemyKicker.textContent = `${gymLeader} squad`;
        if (enemySection) enemySection.textContent = "Leader roster";
        if (btnBack) btnBack.textContent = "Back to Gyms";
        if (btnRetry) btnRetry.textContent = "Retry from Gyms";
        return;
    }

    if (badge) badge.textContent = tSafeArena("battle_arena_badge");
    if (title) title.textContent = tSafeArena("battle_arena_title");
...
function leerSesionGymArena() {
    return leerJSONArena(BATTLE_GYM_SESSION_KEY, null);
}
...
function sincronizarSesionGymArena(data = {}) {
    arenaGymActual = data?.gym || arenaGymActual;
    arenaBattleSessionToken = data?.gym_session_token || arenaBattleSessionToken || null;
    arenaBattleSessionExpiraEn = data?.expira_en || arenaBattleSessionExpiraEn || null;
    arenaBattleSessionTtlSegundos = Number(data?.ttl_segundos || arenaBattleSessionTtlSegundos || 0);
}

function normalizarEquipoSesionGym(data = {}) {
    const equipoUsuario = Array.isArray(data?.equipo_usuario) ? data.equipo_usuario : [];
    const equipoRival = Array.isArray(data?.equipo_rival) ? data.equipo_rival : [];

    arenaPlayerTeam = equipoUsuario
        .slice(0, 6)
        .map((pokemon, index) => normalizarPokemonArena({
            ...pokemon,
            id: Number(pokemon?.id || pokemon?.usuario_pokemon_id || 0),
            hp_actual: Number(pokemon?.hp_actual ?? pokemon?.hp ?? pokemon?.hp_max ?? 0),
            hp_max: Number(pokemon?.hp_max ?? pokemon?.hp ?? 0)
        }, "player", index));

    arenaEnemyTeam = equipoRival
        .slice(0, 6)
        .map((pokemon, index) => normalizarPokemonArena({
            ...pokemon,
            id: pokemon?.id || `gym-enemy-${index + 1}-${pokemon?.pokemon_id || 0}`,
            hp_actual: Number(pokemon?.hp_actual ?? pokemon?.hp ?? pokemon?.hp_max ?? 0),
            hp_max: Number(pokemon?.hp_max ?? pokemon?.hp ?? 0)
        }, "enemy", index));
...
async function iniciarSesionGymArenaDesdeStorage() {
    const data = leerSesionGymArena();
    if (!data || typeof data !== "object") {
        throw new Error("The Gym session is missing. Return to Gyms and start the challenge again.");
    }

    sincronizarSesionGymArena(data);
    normalizarEquipoSesionGym(data);

    if (arenaPlayerTeam.length !== 6 || arenaEnemyTeam.length < 1) {
        throw new Error("The Gym session is incomplete. Return to Gyms and start the challenge again.");
    }
...
function construirResumenRecompensaGymArena(data = null) {
    return {
        ...
        badge: arenaGymActual?.badge || arenaGymActual?.medalla_nombre || "Badge",
...
async function reclamarRecompensaGymArenaApi(victoria = false, turnos = 0) {
    if (!arenaBattleSessionToken) {
        const data = leerSesionGymArena();
        arenaBattleSessionToken = data?.gym_session_token || null;
    }

    if (!arenaBattleSessionToken) {
        throw new Error("The Gym session is no longer available.");
    }

    if (typeof reclamarGymBattle === "function") {
        return await reclamarGymBattle(arenaBattleSessionToken, victoria, turnos);
    }

    return await fetchAuthArena(`${API_BASE}/battle/gyms/recompensa`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            gym_session_token: String(arenaBattleSessionToken || ""),
...
async function cancelarGymArenaApi() {
    if (!arenaBattleSessionToken) {
        const data = leerSesionGymArena();
        arenaBattleSessionToken = data?.gym_session_token || null;
    }

    if (!arenaBattleSessionToken) return null;

    if (typeof cancelarGymBattle === "function") {
        return await cancelarGymBattle(arenaBattleSessionToken);
    }

    return await fetchAuthArena(`${API_BASE}/battle/gyms/cancelar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            gym_session_token: String(arenaBattleSessionToken || "")
...
async function procesarRecompensaGymArena(victoria = false) {
    if (arenaUltimaRecompensaAplicada) return arenaUltimaRecompensaAplicada;
    if (arenaRecompensaEnProceso) return null;
    if (arenaRecompensaProcesada) return arenaUltimaRecompensaAplicada;
    ...
    try {
        ...
        recompensaConfirmada = true;
        registrarActividadArena("battle_end", `gym:${arenaGymActual?.codigo || arenaGymActual?.leader || "gym"}:${victoria ? "victoria" : "derrota"}`).catch(() => {});
        return recompensa;
    } catch (error) {
        console.warn("No se pudo cerrar la sesión del Gym:", error);
        agregarLogArena(error?.message || "The Gym result could not be saved.", tSafeArena("arena_system"));
        return construirResumenRecompensaGymArena(null);
    } finally {
        arenaRecompensaEnProceso = false;
        if (recompensaConfirmada || arenaRecompensaProcesada) {
            limpiarSesionGymArena({ limpiarModo: true });
        }
    }
}

function actualizarMenuModoArena() {
    const battleDesktop = document.querySelector('a[href="battle.html"].menu-dropdown-link');
    const gymsDesktop = document.querySelector('a[href="gyms.html"].menu-dropdown-link');
    const battleMobile = document.querySelector('a[href="battle.html"].menu-mobile-submenu-link');
    const gymsMobile = document.querySelector('a[href="gyms.html"].menu-mobile-submenu-link');
    const battleSubnav = document.querySelector('.menu-subnav-link[href="battle.html"]');
    const gymsSubnav = document.querySelector('.menu-subnav-link[href="gyms.html"]');

    const activeBattle = !esModoGymArena();
    [battleDesktop, battleMobile, battleSubnav].forEach(el => el?.classList.toggle('active', activeBattle));
    [gymsDesktop, gymsMobile, gymsSubnav].forEach(el => el?.classList.toggle('active', esModoGymArena()));
}

function construirDetalleActividadArena() {
    if (esModoBossArena()) {
        return `boss:${arenaBossActual?.codigo || arenaBossActual?.nombre || obtenerCodigoDificultadArena()}`;
    }
    if (esModoGymArena()) {
        return `gym:${arenaGymActual?.codigo || arenaGymActual?.leader || 'gym'}`;
    }
    return `dificultad:${obtenerCodigoDificultadArena()}`;
}

function generarBossEnemigoArena(boss = null) {
    const data = boss || arenaBossActual;
    if (!data) {
        throw new Error(tSafeArena("arena_boss_data_missing"));
    }

    arenaBossActual = { ...data };
    arenaEnemyTeam = [normalizarPokemonArena({
        id: `boss-${data.codigo || data.pokemon_id || "alpha"}`,
        pokemon_id: Number(data.pokemon_id || 0),
        nombre: data.nombre || "Alpha Boss",
        tipo: data.tipo || "Normal",
        nivel: Number(data.nivel || 1),
        hp_max: Number(data.hp_total || data.hp_max || 1),
        hp_actual: Number(data.hp_actual || data.hp_total || data.hp_max || 1),
        ataque: Number(data.ataque || 1),
        defensa: Number(data.defensa || 1),
        velocidad: Number(data.velocidad || 1),
        ataque_especial: Number(data.ataque_especial || data.ataque || 1),
        defensa_especial: Number(data.defensa_especial || data.defensa || 1),
        es_shiny: Boolean(data.es_shiny),
...
function sincronizarSesionBossArena(data = {}) {
    arenaBossActual = data?.boss || arenaBossActual;
    arenaBossEventoActual = data?.evento || data?.boss_evento || arenaBossEventoActual;
    arenaBattleSessionToken = data?.boss_session_token || sessionStorage.getItem(BATTLE_BOSS_SESSION_TOKEN_KEY) || null;
    arenaBattleSessionTtlSegundos = Number(data?.ttl_segundos || arenaBattleSessionTtlSegundos || 0);
    arenaBattleSessionExpiraEn = construirExpiraBossArena(
        arenaBattleSessionTtlSegundos,
        data?.expira_en || sessionStorage.getItem(BATTLE_BOSS_SESSION_EXPIRES_KEY) || null
    );

    if (arenaBattleSessionToken) {
        sessionStorage.setItem(BATTLE_BOSS_SESSION_TOKEN_KEY, String(arenaBattleSessionToken));
    }

    if (arenaBattleSessionExpiraEn) {
        sessionStorage.setItem(BATTLE_BOSS_SESSION_EXPIRES_KEY, String(arenaBattleSessionExpiraEn));
    }
...
async function iniciarSesionBossArena() {
    ...
    const data = await iniciarBossArenaApi(usuarioPokemonIds, true);
    if (!data?.ok || !data?.boss_session_token || !data?.boss) {
        throw new Error(data?.mensaje || tSafeArena("arena_boss_start_error"));
    }

    sincronizarSesionBossArena(data);
    return data;
}

function calcularDamageBossArena() {
    if (!esModoBossArena()) return 0;
    const boss = arenaEnemyTeam[0] || null;
    const hpTotal = Number(arenaBossActual?.hp_total || boss?.hp_max || 0);
    const hpActual = Math.max(0, Number(boss?.hp_actual || 0));
...
function construirResumenRecompensaBossArena(data = null) {
    ...
    return {
        ...
        pokedolares: Number(data?.pokedolares_ganados || 0),
        rewardTier,
        rewardTierLabel: tSafeArena(`boss_tier_${rewardTier}`),
...
async function procesarRecompensaBossArena(victoria = false) {
    if (arenaUltimaRecompensaAplicada) return arenaUltimaRecompensaAplicada;
    if (arenaRecompensaEnProceso) return null;
    if (arenaRecompensaProcesada) return arenaUltimaRecompensaAplicada;
    ...
    try {
        ...
        agregarLogArena(
            tfArena("arena_boss_rewards_log", {
                tier: recompensa.rewardTierLabel,
                damage: recompensa.damageTotal,
                bossHp: recompensa.bossHpTotal,
                coins: recompensa.pokedolares,
                exp: recompensa.exp
            }),
        ...
        agregarLogArena(error?.message || tSafeArena("arena_boss_reward_error"), tSafeArena("arena_system"));
        return construirResumenRecompensaBossArena(null);
    } finally {
        arenaRecompensaEnProceso = false;
    ...
    return `
        <article class="arena-mini-card ${tipoClase} ${isActive ? (isEnemy ? "enemy-active" : "active") : ""} ${pokemon.defeated ? "fainted" : ""}">
            <div class="arena-mini-card-top">
                ...
                ${isActive ? `<span class="arena-mini-focus-badge ${isEnemy ? "enemy" : "player"}">${isEnemy ? (esModoBossArena() ? tSafeArena("battle_mode_boss_title") : esModoGymArena() ? "Gym rival" : tSafeArena("arena_mini_rival")) : tSafeArena("arena_mini_active")}</span>` : ""}
...
function mostrarDanioFlotanteArena(esJugador, valor, multiplicador = 1) {
    ...
    let multText = "";
    if (Math.abs(Number(multiplicador || 1) - 1) > 0.001) {
        multText = ` x${Number(multiplicador).toFixed(Number.isInteger(multiplicador) ? 0 : 2).replace(/\.00$/, "")}`;
    }

    pop.textContent = `-${valor}${multText}`;
...
function mostrarResultadoArena(victoria = true, recompensa = null, soloRefresco = false) {
    ...
    if (esModoBossArena()) {
        const recompensaBoss = recompensa || construirResumenRecompensaBossArena();
        if (victoria) {
            icon.textContent = "👑";
            icon.style.background = "#fef3c7";
            icon.style.color = "#b45309";
            title.textContent = tSafeArena("arena_boss_victory_title");
            text.textContent = tfArena("arena_boss_result_victory_text", {
                boss: arenaBossActual?.nombre || tSafeArena("battle_mode_boss_title"),
                tier: recompensaBoss.rewardTierLabel || tSafeArena("boss_tier_alpha"),
                damage: recompensaBoss.damageTotal || 0,
                bossHp: recompensaBoss.bossHpTotal || 0,
            ...
            title.textContent = tSafeArena("arena_boss_defeat_title");
            text.textContent = tfArena("arena_boss_result_defeat_text", {
                boss: arenaBossActual?.nombre || tSafeArena("battle_mode_boss_title"),
                tier: recompensaBoss.rewardTierLabel || tSafeArena("boss_tier_bronze"),
                damage: recompensaBoss.damageTotal || 0,
                bossHp: recompensaBoss.bossHpTotal || 0,
                coins: recompensaBoss.pokedolares || 0,
                exp: recompensaBoss.exp || 0
    ...
    if (esModoGymArena()) {
        ...
        const gymLeader = arenaGymActual?.leader || arenaGymActual?.lider_nombre || recompensaGym?.nombre || "Gym Leader";
        const gymBadge = arenaGymActual?.badge || arenaGymActual?.medalla_nombre || recompensaGym?.badge || "Badge";
        if (victoria) {
            icon.textContent = "🏅";
            icon.style.background = "#fef3c7";
            icon.style.color = "#b45309";
...
async function iniciarBossArenaApi(usuarioPokemonIds = [], guardarEquipo = true) {
    if (typeof iniciarBossMundo === "function") {
        return await iniciarBossMundo(usuarioPokemonIds, guardarEquipo);
    }

    return await fetchAuthArena(`${API_BASE}/battle/boss/iniciar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
async function reclamarRecompensaBossArenaApi(damageTotal = 0, bossDerrotado = false, turnos = 0) {
    if (!arenaBattleSessionToken) {
        arenaBattleSessionToken = sessionStorage.getItem(BATTLE_BOSS_SESSION_TOKEN_KEY) || null;
    }

    if (typeof reclamarBossMundo === "function") {
        return await reclamarBossMundo(arenaBattleSessionToken, damageTotal, bossDerrotado, turnos);
    }

    return await fetchAuthArena(`${API_BASE}/battle/boss/recompensa`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            boss_session_token: String(arenaBattleSessionToken || ""),
